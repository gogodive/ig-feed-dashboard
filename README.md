...
자사 인스타 계정(config.yaml 등록, 현재 7개)의 피드를 매일 아침 7시(KST)에 수집해
...
### 4. (선택) 노션 상태 콜아웃 자동 갱신
허브 페이지 최상단 콜아웃에 매일 실행 결과를 한 줄로 기록합니다.
설정하지 않아도 수집·배포는 정상 동작합니다.

1. https://www.notion.so/my-integrations → **내부 통합 생성** → 토큰 복사
   - 필요한 기능: 콘텐츠 읽기 + **콘텐츠 업데이트**
2. 허브 페이지 우측 상단 ⋯ → **연결** → 위 통합 선택
3. GitHub Secrets 에 `NOTION_TOKEN` 등록
4. `config.yaml` 의 `notion.hub_page_id` 가 대상 페이지 ID 인지 확인
5. 페이지 최상단에 **콜아웃 블록**이 하나 있어야 합니다(첫 콜아웃을 갱신함)

상태 표기: `✅ 전원 성공` / `⚠️ 일부 계정 실패(계정명 표기)` / `🚨 전원 실패`
