...
def build_status(accounts: list[dict], generated_at: datetime) -> tuple[str, str]:
    ...
    이번 실행에서 갱신된 계정 = fetched_at 이 generated_at 과 같은 시각인 계정.
    실패 계정은 이전 데이터를 유지하므로 시각이 다르거나 없다.
    """
    ok, failed = [], []
    for a in accounts:
        ...
        if fetched:
            try:
                same = datetime.fromisoformat(fetched) == generated_at
            except ValueError:
                same = False
        (ok if same else failed).append(a.get("brand") or a.get("username"))

    stamp = generated_at.astimezone(KST).strftime("%Y-%m-%d %H:%M")
    total = len(accounts)
    posts = sum(len(a.get("posts", [])) for a in accounts)

    if not failed:
        return "✅", f"{stamp} 갱신 완료 · {total}개 계정 · 게시물 {posts:,}개"
    if not ok:
        return "🚨", (f"{stamp} 갱신 실패 · 0/{total}개 계정 "
                      f"— 토큰 만료 또는 권한 문제일 수 있습니다 (화면은 이전 데이터)")
    names = ", ".join(failed)
    return "⚠️", (f"{stamp} 갱신 · {len(ok)}/{total}개 계정 성공 "
                  f"— 실패: {names} (해당 계정은 이전 데이터)")
